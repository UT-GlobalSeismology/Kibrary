...
final class ANISOtime {
    ...
    private static void downloadManual() throws IOException, NoSuchAlgorithmException, URISyntaxException {
        ...
        Path path = FileUtils.download(new URL(USER_GUIDE_URL));
        if (Files.exists(localPath)) {
            String localSum = GadgetUtils.checksum(localPath, "SHA-256");
            String cloudSum = GadgetUtils.checksum(path, "SHA-256");
