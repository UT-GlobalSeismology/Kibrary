...
public class GYPSUM {
    ...
    public static void main(String[] args) {
        double[] radiiForMTZ = new double[] {6021.0, 5981, 5929.75, 5867.25, 5804.75, 5742.25, 5671.0, 5591.0};

        GYPSUM model = new GYPSUM();

        for (double r : radiiForMTZ) {
            Path outpath = Paths.get("/Users/Anselme/Dropbox/Kenji/UPPER_MANTLE/global3D/GYPSUM/" + r + ".dat");
            double[][] layer = model.getLayerAtDepth(6371 - r);
            try {
                model.writeLayer(outpath, layer);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
...
//		double dR = 10.;
//		GYPSUM model = new GYPSUM(dR);
//
//		Path outpath = Paths.get("/Users/Anselme/Dropbox/Kenji/UPPER_MANTLE/global3D/GYPSUM/gypsum.txt");
//
//		try {
//			double rTrunc = 5000.;
...
//						dvs = 0.;
                    pw.printf("%.2f %.2f %.2f %.3f %.4f\n",lonMantle[ilon], latMantle[ilat]
                        ...
                        , prem.getAtRadius(VariableType.Vsh, 6371 - depthMantle[idepth]));
                }
            }
        }
    ...
    public void writeLayer(Path outpath, double[][] layer) throws IOException {
        PrintWriter pw = new PrintWriter(outpath.toFile());
            for (int ilon = 0; ilon < nLonMantle; ilon++) {
                for (int ilat = 0; ilat < nLatMantle; ilat++) {
                    double dvs = layer[ilon][ilat];
                    pw.printf("%.2f %.2f %.3f\n",lonMantle[ilon], latMantle[ilat]
                        , dvs);
                }
            }
...
//			iRmax = nDepthMantle - 1;
    ...
    public double[][] getLayerAtDepth(double depth) {
        double[][] dvsLayer = new double[nLonMantle][nLatMantle];
...
//		String fileName = SpcSAC.class.getClassLoader().getResource("US-SL-2014_percent.nc").toString();
        String fileName = "/Users/Anselme/Dropbox/Kenji/UPPER_MANTLE/global3D/GYPSUM/GYPSUM_percent.nc";
