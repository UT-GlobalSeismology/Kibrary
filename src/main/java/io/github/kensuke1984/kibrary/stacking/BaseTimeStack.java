...
public class BaseTimeStack implements Stack {
     ...
     *
     * @param trace    　元になるTrace (変更されない)
     * @param baseTime 基準時刻。　０になる
    ...
    @Override
    public Trace stack(String station, GlobalCMTID globalCMTID, SACComponent component, WaveformType type,
                       Trace trace) {
        Key key = new Key(station, globalCMTID, component);
        if (!baseTimeMap.containsKey(key)) throw new RuntimeException(
                "The data for " + station + "." + globalCMTID + "." + component + " doesn't exist.");
     ...
     */
    public void addBaseTime(String station, GlobalCMTID id, SACComponent component, double baseTime) {
        Key key = new Key(station, id, component);
        if (baseTimeMap.containsKey(key))
            throw new RuntimeException("The data for " + station + "." + id + "." + component + " already exists.");
