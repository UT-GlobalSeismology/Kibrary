 ...
 * @version 0.0.1
 */
class SACVarianceComparator implements DataComparator<SACFileData[]> {

    private final double OBS2;
    private final SACFileData[] OBSERVED_DATASET;
    private final double SIGMA = 0.5;


    SACVarianceComparator(Path obsDir) throws IOException {
        OBSERVED_DATASET = readObserved(obsDir);
        OBS2 = Arrays.stream(OBSERVED_DATASET).map(SACFileData::getData).flatMapToDouble(Arrays::stream)
                .reduce(0, (i, j) -> i + j * j);
    }

    private SACFileData[] readObserved(Path obsDir) throws IOException {
        ...
        SACFileData[] dataset = new SACFileData[names.length];
    ...
    private double computeVariance(SACFileData[] synSAC) {
        double numerator = 0;
     ...
     */
    private boolean hasProblems(SACFileData[] dataset) {
    ...
    @Override
    public double likelihood(SACFileData[] data) {
