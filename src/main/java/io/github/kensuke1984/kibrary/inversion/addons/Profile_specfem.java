...
public class Profile_specfem {
//	public static void main(String[] args) {
//		Path inversionResultPath = Paths.get(".");
//		
//		try {
//			Path profilePath = inversionResultPath.resolve("profile_specfem");
//			Path waveformIDinitPath = profilePath.relativize(inversionResultPath.resolve(args[0]));
//			Path waveforminitPath = profilePath.relativize(inversionResultPath.resolve(args[1]));
//			Path waveformID3DPath = profilePath.relativize(inversionResultPath.resolve(args[2]));
//			Path waveform3DPath = profilePath.relativize(inversionResultPath.resolve(args[3]));
//			
...
//			
//			Path stackRoot = inversionResultPath.resolve("stack_specfem/");
...
//			
//			List<BasicID> initSynList = Arrays.stream(dvectorInit.getSynIDs()).collect(Collectors.toList());
...
//				List<BasicID> tmpInitSyn = initSynList.stream().filter(id -> id.getGlobalCMTID().equals(event)).collect(Collectors.toList());
//				List<BasicID> tmpSpecfemSyn = specfemSynList.stream().filter(id -> id.getGlobalCMTID().equals(event)).collect(Collectors.toList());
//				
...
//					
//					for (int i = 0; i < tmpObs.size(); i++) {
//						BasicID id = tmpObs.get(i);
////						i.incrementAndGet();
//						try {
//							RealVector bornVector = tmpSpecfemSyn.get(i).getTrace().getYVector();
