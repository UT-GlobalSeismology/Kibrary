...
//			}
			...
			Comparator<BasicID> compareByDistance = (BasicID o1, BasicID o2) -> {
				double d1 = o1.getGlobalCMTID().getEventData().getCmtPosition().computeEpicentralDistanceRad(o1.getObserver().getPosition());
				double d2 = o2.getGlobalCMTID().getEventData().getCmtPosition().computeEpicentralDistanceRad(o2.getObserver().getPosition());
			...
			for (GlobalCMTID event : events) {
				...
				try {
					...
					String header = String.join("\n", "set terminal postscript enhanced color font \"Helvetica,14\"",
							"set output \"" + event + ".ps\"",
							"unset key",
							"set xlabel 'Time aligned on PcP-wave arrival (s)'",
...
//						i.incrementAndGet();
						try {
							...
							double distance = id.getGlobalCMTID().getEventData().getCmtPosition().computeEpicentralDistanceRad(id.getObserver().getPosition())
									* 180. / Math.PI;
...
//								stationObsNorm.put(sta, tmpObsNorm);
				...
				for (BasicID id : tmpObs) {
					...
					double distance = id.getGlobalCMTID().getEventData().getCmtPosition().computeEpicentralDistanceRad(id.getObserver().getPosition())
							* 180. / Math.PI;
					double azimuth = Math.toDegrees(id.getGlobalCMTID().getEventData().getCmtPosition().computeAzimuthRad(id.getObserver().getPosition()));
					int i = (int) (distance);
					int j = (int) (azimuth / dAz);
					
					timetool.calculate(distance);
					Arrival pcpArrival = timetool.getArrival(0);
					if (!pcpArrival.getName().equals("PcP")) {
						System.out.println("Problem computing PcP time for " + id);
						continue;
					}
				...
				try (PrintWriter pw = new PrintWriter(Files.newBufferedWriter(outPlot, StandardOpenOption.CREATE, StandardOpenOption.APPEND))) {
					...
					pw.println("set xlabel 'Time aligned on PcP-wave arrival (s)'");
