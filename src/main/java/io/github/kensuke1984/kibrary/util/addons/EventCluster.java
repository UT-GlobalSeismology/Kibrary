...
//		Path outpath = Paths.get("/work/anselme/CA_ANEL_NEW/VERTICAL/syntheticPREM_Q165/filtered_stf_6-200s/map/azimuthSeparation_cluster_pcp.inf");
        ...
        List<BasicID> waveformIDs = BasicIDFile.read(waveformIDPath, false);
        Map<Observer, Set<Integer>> stationClusterMap = new HashMap<>();
        waveformIDs.stream().forEach(id -> stationClusterMap.put(id.getObserver(), new HashSet<>()));
        waveformIDs.stream().forEach(id -> {
...
//			System.out.println(ilon + " " + ilat);
    ...
    public int getAzimuthIndex(HorizontalPosition pos) {
        int iaz = -1;
        double azimuth = Math.toDegrees(centerPosition.computeAzimuthRad(pos));
