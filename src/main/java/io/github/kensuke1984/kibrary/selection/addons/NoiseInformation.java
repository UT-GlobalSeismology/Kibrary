...
public class NoiseInformation {
    ...
    public static void main(String[] args) throws IOException {
        Path spcAmpPath = Paths.get(args[0]);
        Path spcNoisePath = Paths.get(args[1]);
        Path outpath = Paths.get("signalNoise.dat");


        List<BasicID> waveforms = BasicIDFile.read(spcAmpPath, true);
        List<BasicID> noises = BasicIDFile.read(spcNoisePath, true).stream()
