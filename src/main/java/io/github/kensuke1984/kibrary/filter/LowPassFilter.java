...
public class LowPassFilter extends ButterworthFilter {
     ...
     * @param omegaP &omega;<sub>P</sub> 透過域の最大角周波数 maximum angular frequency in
     *               permissible region
     * @param omegaS &omega;<sub>S</sub> 遮断域の最小角周波数 minimum angular frequency in
     *               stopping region
     */
    public LowPassFilter(double ap, double as, double omegaP, double omegaS) {
        ...
        if (omegaS < 0 || omegaS <= omegaP) throw new IllegalArgumentException("Input parameters are invalid");
        setSigmaSoverSigmaP();
        setN();
        setC();
