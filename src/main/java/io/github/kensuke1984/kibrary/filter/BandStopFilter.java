...
public class BandStopFilter extends ButterworthFilter {
     ...
     * @param omegaH  &omega;<sub>H</sub> 高周波透過域の最小角周波数 minimum &omega; for higher
     *                permissible region
     * @param omegaL  &omega;<sub>L</sub> 低周波透過域の最大角周波数 maximum &omega; for lower
     *                permissible region
     * @param omegaSh &omega;<sub>Sh</sub> 遮断域の最大角周波数 maximum &omega; for blocking
     *                region
     */
    public BandStopFilter(double ap, double as, double omegaH, double omegaL, double omegaSh) {
        ...
        if (omegaH <= omegaSh || omegaSh <= omegaL) throw new IllegalArgumentException("Input parameters are invalid");
        setSigmaSoverSigmaP();
        setN();
        setC();
        setOmegaSl();
...
//        System.out.println("G" + g + " n" + n);
     ...
     * a<sub>0</sub> =1, a<sub>1</sub> = a<sub>2</sub> = 1
     *
