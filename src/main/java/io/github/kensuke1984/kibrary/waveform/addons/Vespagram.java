...
public class Vespagram {
    ...
    public static void main(String[] args) {
        Path basicPath = Paths.get(args[0]);
        ...
        try {
            List<BasicID> waveforms = BasicIDFile.read(basicPath, true);
            Set<GlobalCMTID> events = waveforms.stream().map(BasicID::getGlobalCMTID).collect(Collectors.toSet());
    ...
    public Vespagram(List<BasicID> waveforms, GlobalCMTID event, double ds, double smax) {
        this.event = event;
        sort(waveforms);
        ...
        for (int i = 0; i < obsIDs.length; i++) {
            double distance = Math.toDegrees(obsIDs[i].getGlobalCMTID().getEventData().getCmtPosition().computeEpicentralDistanceRad(obsIDs[i].getObserver().getPosition()));
        ...
        this.ds = ds;
        this.smax = smax;
        samplingHz = waveforms.get(0).getSamplingHz();
    }
    ...
    public double[][] computeSyn(double distanceBin) {
        ...
        for (int is = 0; is < ns; is++) {
            ...
            for (int i = 0; i < id2bin.length; i++) {
                ...
                double distance = Math.toDegrees(synIDs[i].getGlobalCMTID().getEventData().getCmtPosition().computeEpicentralDistanceRad(synIDs[i].getObserver().getPosition()));
                double timeshift = smax * deltaDistance - slowness * (distance - distance0);
                int shift = (int) (timeshift * samplingHz);
    ...
    private void sort(List<BasicID> ids) {
        Predicate<BasicID> chooser = id -> id.getGlobalCMTID().equals(event);

        List<BasicID> obsList = ids.stream().filter(id -> id.getWaveformType() == WaveformType.OBS)
        ...
        List<BasicID> synList = ids.stream().filter(id -> id.getWaveformType() == WaveformType.SYN)
                .filter(chooser::test).collect(Collectors.toList());
