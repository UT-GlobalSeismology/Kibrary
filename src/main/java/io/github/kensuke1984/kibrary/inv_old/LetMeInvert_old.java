...
//			System.out.println(azimuthRange[0] + " " + azimuthRange[1]);
            ...
            chooser = id -> {
                ...
                double distance = id.getGlobalCMTID().getEventData()
                        .getCmtPosition().computeEpicentralDistance(id.getObserver().getPosition())
                        * 180. / Math.PI;
...
//				applyConditionnerBulk();
            ...
            if (modelCovariance) {
                ...
                if (inverseMethods.contains(InverseMethodEnum.FAST_CONJUGATE_GRADIENT) || inverseMethods.contains(InverseMethodEnum.FAST_CONJUGATE_GRADIENT_DAMPED))
                    ...
                    try (PrintWriter pw = new PrintWriter(Files.newBufferedWriter(outPath.resolve("ata.lst")))) {
                        for (int i = 0; i < ata.getColumnDimension(); i++) {
                            for (int j = 0; j < ata.getColumnDimension(); j++) {
                                pw.print(ata.getEntry(i, j) + " ");
                            }
                            pw.println();
                        }
                    }
        ...
        try (PrintWriter pw = new PrintWriter(Files.newBufferedWriter(outPath, StandardOpenOption.CREATE_NEW))) {
            ...
            Arrays.stream(obsIDs).forEach(id -> {
                ...
                double epicentralDistance = Math
                        .toDegrees(observer.getPosition().computeEpicentralDistance(event.getCmtPosition()));
                double azimuth = Math.toDegrees(observer.getPosition().computeAzimuth(event.getCmtPosition()));
                pw.println(
                        observer + " " + observer.getPosition() + " " + id.getGlobalCMTID() + " " + event.getCmtPosition()
