        ...
        case RECIPROCAL_PcP:
            WEIGHTING_FUNCTION = (obs, syn) -> {
                ...
                double distance = Math.toDegrees(obs.getGlobalCMTID().getEventData().getCmtPosition()
                        .computeEpicentralDistanceRad(obs.getObserver().getPosition()));
                double a = 3.;
...
//		return tmp * tmp;
    ...
    private double weightingEpicentralDistanceDpp(BasicID obs) {
        double weight = 1.;
        double distance = obs.getGlobalCMTID().getEventData().getCmtPosition().computeEpicentralDistanceRad(obs.getObserver().getPosition()) * 180. / Math.PI;
...
//				,{84.0,1.58},{86.0,1.81},{88.0,2.49},{90.0,2.25},{92.0,3.57},{94.0,3.72},{96.0,7.48} };
    ...
    public double weightingAzimuthDpp(BasicID obs) {
        double weight = 1.;
        double azimuth = obs.getGlobalCMTID().getEventData().getCmtPosition().computeAzimuthRad(obs.getObserver().getPosition()) * 180. / Math.PI;
