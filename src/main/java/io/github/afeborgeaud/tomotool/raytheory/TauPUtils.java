...
public class TauPUtils {
    ...
    private FullPosition sourceLoc;
    ...
    public void setCMTLocation(FullPosition sourceLoc) {
        timetool.setSourceDepth(6371. - sourceLoc.getR());
        this.sourceLoc = sourceLoc;
    }

    public double getEpicentralDistance(HorizontalPosition staPos) {
        return Math.toDegrees(sourceLoc.computeGeographicalDistanceRad(staPos));
    }
...
//		double geographicalDistance = Math.toDegrees(sourceLoc.getGeographicalDistance(staPos));
        double distance = Math.toDegrees(sourceLoc.computeEpicentralDistanceRad(staPos));
        double azimuthtmp = Math.toDegrees(sourceLoc.computeGeographicalAzimuthRad(staPos));
...
//	}
    ...
    public void process(List<HorizontalPosition> scatterPoints, double rayparam, TimeDist[] pathPoints,
        ...
        //Process raypath
        double azimuth = Math.toDegrees(sourceLoc.computeGeographicalAzimuthRad(staPos));
        FullPosition[] raypath = new FullPosition[pathPoints.length];
        for (int i = 0; i < pathPoints.length; i++) {
            TimeDist td = pathPoints[i];
            double lat = SphericalCoords.latFor(sourceLoc.getLatitude(), sourceLoc.getLongitude(), td.getDistDeg(), azimuth);
            double lon = SphericalCoords.lonFor(sourceLoc.getLatitude(), sourceLoc.getLongitude(), td.getDistDeg(), azimuth);
            raypath[i] = new FullPosition(lat, lon, earth_radius - td.getDepth());
        }
    ...
    public static class Ray {
        ...
        private FullPosition[] raypath;

        private String phaseName;

        public Ray(String phaseName, List<ScatterPoint> scatterPoints, FullPosition[] raypath) {
        ...
        public FullPosition[] getRaypath() {
            return raypath;
        }
