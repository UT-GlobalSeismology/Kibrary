...
public class AlignWindows {
    ...
    List<TimewindowData> alignedTimewindows;
    ...
    public AlignWindows(List<TimewindowData> timewindows, Path sacpath, double timeBeforePeak, double timeAfterPeak, double minCc) {
        this.timewindows = timewindows;
        alignedTimewindows = new ArrayList<TimewindowData>();
    ...
    public static void main(String[] args) {
        ...
        try {
            TimewindowDataFile.write(alignwindows.getAlignedTimewindows().stream().collect(Collectors.toSet()), outpath);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
...
//			polarizedShifts.add(new PolarizedShift(0, 1, 1));
        ...
        for (GlobalCMTID event : events) {
            List<TimewindowData> eventTimewindows = timewindows.parallelStream().filter(tw -> tw.getGlobalCMTID().equals(event))
            ...
            for (int i = 0; i < eventTimewindows.size(); i++) {
                TimewindowData window = eventTimewindows.get(i);
                String sacnameString = window.getGlobalCMTID() + "/" + window.getObserver().toString() + "." + window.getGlobalCMTID()
...
//			polarizedShifts.add(new PolarizedShift(0, 1, 1));
        ...
        for (GlobalCMTID event : events) {
            List<TimewindowData> eventTimewindows = alignedTimewindows.parallelStream().filter(tw -> tw.getGlobalCMTID().equals(event))
                    .collect(Collectors.toList());
            RealVector wavelet = new ArrayRealVector((int) (eventTimewindows.get(0).getLength() * samplingHz) + 1);
...
//					System.out.println(pointShift + " " + wavelet0.getDimension() + " " + tmpWaveform.getDimension());
    ...
    private void removeBadWindows() {
        ...
        List<TimewindowData> tmpwindows = new ArrayList<>();
    ...
    private void computeAlignTimewindows() {
        ...
        for (int i = 0; i < timewindows.size(); i++) {
            TimewindowData window = timewindows.get(i);
            String sacnameString = window.getGlobalCMTID() + "/" + window.getObserver().toString() + "." + window.getGlobalCMTID()
                + "." + window.getComponent() + "sc";
            Path syn = sacpath.resolve(sacnameString);
            ...
            try {
                ...
                TimewindowData tmpwindow = new TimewindowData(t0, t1,
                        window.getObserver(), window.getGlobalCMTID(),
    ...
    private void computePhase2AlignTimewindows() {
        for (int i = 0; i < alignedTimewindows.size(); i++) {
            TimewindowData window = alignedTimewindows.get(i);
            String sacnameString = window.getGlobalCMTID() + "/" + window.getObserver().toString() + "." + window.getGlobalCMTID()
                + "." + window.getComponent() + "sc";
            Path syn = sacpath.resolve(sacnameString);
    ...
    public List<TimewindowData> getAlignedTimewindows() {
