...
//			minCc = -1;
            ...
            List<TimewindowData> timewindows = readAndAlignTimewindows(timewindowPath, modelRefName, Paths.get(modelRefName),
...
//			compute_phase(timewindowsForLargestCluster, modelRefName, modelName, phaseName);
            ...
            List<TimewindowData> timewindows = TimewindowDataFile.read(timewindowPath).stream().collect(Collectors.toList());
    ...
    public CrossCorrelationTimeshift(List<TimewindowData> timewindows, Path obsPath, Path synPath, boolean convolute) {
        obsNames = new ArrayList<>();
        synNames = new ArrayList<>();
        this.timewindows = timewindows;
        for (TimewindowData window : timewindows) {
            String obsString = window.getGlobalCMTID() + "/" + window.getObserver().toString() + "." + window.getGlobalCMTID()
...
//						/ (traceAfter.getYVector().getNorm() / 5.);
    ...
    private static List<TimewindowData> readAndAlignTimewindows(Path timewindowPath, String refmodel, Path sacpath,
            double timeBeforePeak, double timeAfterPeak, double minCc) {
        List<TimewindowData> timewindows = null;
        try {
            timewindows = TimewindowDataFile.read(timewindowPath).stream().collect(Collectors.toList());
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public static void computePKKKP() {
        ...
        try {
            List<TimewindowData> timewindows
                = TimewindowDataFile.read(timewindowInformationPath).stream().collect(Collectors.toList());
            ...
            List<io.github.afeborgeaud.tomotool.raytheory.TraveltimeData> rayMearuements = Compute.compute_phase(timewindows, seismic3Dmodel, "PKKKKP");

            Path outpath = Paths.get("/work/anselme/TOPO/ETH/synthetics/correlationTimeshift_topo_PKKKKP.dat");
            PrintWriter pw = new PrintWriter(outpath.toFile());
            for (int i = 0; i < timewindows.size(); i++) {
                Measurement m = measurements.get(i);
                io.github.afeborgeaud.tomotool.raytheory.TraveltimeData mRay = rayMearuements.get(i);
    ...
    public static void computeScS() {
        ...
        try {
            ...
            Seismic3Dmodel seismic3Dmodel = new TK10();
            List<io.github.afeborgeaud.tomotool.raytheory.TraveltimeData> rayMearuements = Compute.compute_phase(timewindows, seismic3Dmodel, "ScS");
    ...
    public static void compute_phase(List<TimewindowData> timewindows, String modelRefName, String modelName, String phaseName) {
        ...
        try {
            ...
            List<io.github.afeborgeaud.tomotool.raytheory.TraveltimeData> rayMearuements = Compute.compute_phase(timewindows, seismic3Dmodel, phaseName);
            ...
            for (int i = 0; i < timewindows.size(); i++) {
                ...
                pw.println(mRay.getTraveltimePerturbation() + " " + -m.getTimeshift()
                    ...
                    + " " + point.getPosition() + " " + mRay.getPhaseName() + " " + mRay.getObserver().toString() + " " + mRay.getGlobalCMTID());
    ...
    public static void compute_windows(List<TimewindowData> timewindows, String modelRefName, String modelName) {
        String modelSyn = modelRefName;
        String modelObs = modelName;
        Path workDir = Paths.get(".");
        Path obsPath = workDir.resolve(modelObs);
        ...
        try {
            ...
            for (int i = 0; i < timewindows.size(); i++) {
                ...
                pw.println(-m.getTimeshift()
                    ...
                    + " " + timewindows.get(i).getObserver().toString() + " " + timewindows.get(i).getGlobalCMTID());
            }
            pw.close();
